...
当前深色界面保留原有蓝色系，只借鉴用户参考图的明暗强度关系：标题栏最亮，侧栏居中，选中会话标签与工作区、终端使用相同深蓝底色。参考图不作为改为中性灰色系的依据。
...
--border-default:       #2A3A51
--border-subtle:        #1C283B
--border-strong:        #354963
...
- 新增 `Tunnels / 隧道` 同级管理页，详细交互见 `docs/SSH_TUNNELS.md`。使用全宽、按 SSH 服务器分区且始终展开的紧凑表格，不提供分组折叠。提供搜索、服务器/监听状态筛选、启停与复制地址。选中规则后按需展示 320px 详情栏，窄屏置于下方；不占用终端工作区或插入 Shell 命令。
- 隧道页以蓝色层次、局部精细网格、青蓝端口节点与状态描边强化控制台科技感，不使用虚假流量动画。顶部统计全部规则、正在监听、当前连接及需要关注的规则数，不随列表筛选改变；读取失败或加载中显示未获取，不伪装为零。详情以本地入口 → SSH 服务器 → 内网目标展示路径，未检测的目标不以成功颜色暗示可达。兼容深浅主题与窄屏。
- 隧道表单分离名称、SSH 服务器、远端目标与本地入口；Rust 固定绑定 `127.0.0.1`。保存不自动启动；点击启动在隧道页复用认证弹窗，建立不含 Shell/PTY 的专用后台连接，不创建或切换终端标签。同一 SSH 配置的规则共享后台连接，最后一条停止后释放；关闭终端不影响转发。监听状态与带时间戳的 TCP 目标检测分别呈现，不用“已监听”暗示应用健康。“打开终端”是独立的显式操作。

- 左侧 `Servers / 服务器` 打开独立服务器管理页，使用全宽自适应主机网格（最小卡片宽 252px），可切换紧凑列表并记住视图偏好。紧凑标题栏展示主机总数、真实已连接数量和新增入口；搜索、全部／最近连接筛选与视图切换位于同一工具栏。分组保留折叠与排序，最近连接采用动态筛选，不重复展示主机。
- 未选择主机时不占用详情空间；选中后显示 300px 可关闭详情侧栏。窄屏详情位于下方，列表与详情独立滚动，避免固定列表宽度浪费工作区。
- 主机卡片和列表行整体可拖拽，不显示独立手柄；连接按钮和操作菜单不触发拖拽，单击查看与双击连接保持不变。可拖到分组标题或内容区域；拖动时额外显示全部分组与“未分组”落点，支持空分组、折叠分组及搜索／最近连接视图。移动后追加到目标组末尾，保存成功才更新归属，不显示成功提示；失败时保留错误提示。Esc、移出落点或取消手势不修改数据。键盘用户可通过现有编辑表单修改分组；触控保留纵向滚动。不改变 Tauri 原生文件拖放配置，保留 SFTP 文件上传行为。
- 左侧不再提供重复的 `Sessions / 会话` 入口；通过标题栏会话标签进入对应工作区，继续承载 Terminal、Files、Monitor、Operations、Deploy 和右侧 Agent / Inspect 面板。服务器管理列表不再常驻会话页。
- 管理页单击服务器只选择并显示信息；“打开会话”或双击进入会话页，已连接服务器激活现有会话。会话页的新增标签入口返回管理页选择服务器，同服务器多终端仍可通过复制标签或右侧新建终端操作打开。
- 切换页面保留终端实例与 Channel 绑定，不断开 SSH；后台终端不抢焦点，返回会话页后重新适配尺寸。
- 标题栏排列为 Logo、紧凑搜索框、会话标签、拖动区域、隧道动态汇总、窗口控制；不显示新增标签按钮。会话标签在管理页仍可访问，点击后进入对应会话，关闭／复制／重命名行为保留。
- 隧道动态汇总仅在存在运行中的监听隧道时显示，全部停止或状态读取不可用时隐藏。标题栏入口仅显示图标与数量，最小宽度 48px，上行绿色、下行品牌蓝紫色高亮从边框相对位置反向流动，同时收发时呈现双向对流，不显示内部横线；点击展开采样上下行速率、连接数、主机与端口路径，并可进入隧道管理。两段边框动画分别由 Rust 收发字节计数增量触发；首次采样、重启、计数重置不产生虚假流量，空闲时恢复静态边框。窄屏保留紧凑入口，减少动态效果设置下以静态高亮边框代替动画。
- Terminal / Files / Dashboard / Operations / Deploy 与 AI Agent / Inspect 位于标题栏下方的同一行，统一为 40px 高。右侧面板从该行开始，不再低于会话标签栏。
- 终端搜索、复制选中内容、粘贴按钮位于上述工作区标签栏最右侧，仅当前会话的 Terminal 视图激活时显示。搜索框仍在终端右上角展开；窄屏时左侧标签横向滚动，右侧操作区保持可见。
- 全局搜索通过现有连接流程打开目标服务器；设置弹窗由应用层承载，各页面均可访问。无会话时可从空状态返回服务器管理页选择服务器，该页保留新建连接入口。移动端菜单提供服务器、隧道及设置入口，会话通过顶部可横向滚动的标签切换。

以下五区结构保留为工作区视觉基线；Resource Explorer 的服务器管理内容归属独立管理页。
...
当前导航仅包含 Servers、SSH Tunnels 和 Settings；会话由标题栏标签访问。下述概念结构与 Phase 列表为历史设计，不要求恢复 Sessions 侧栏入口。
...
当前服务器列表已移入独立管理页，使用全宽自适应布局，不再提供旧 Explorer 列表宽度调整；会话页不再提供 Explorer 显隐按钮与 `Ctrl/Cmd + B`。下述 Explorer 显隐项属于历史多栏布局，Inspector 的 `Ctrl/Cmd + Shift+B` 仅在会话页生效。
