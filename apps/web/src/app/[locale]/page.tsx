...
import { Activity, ArrowRight, ArrowUpRight, Box, Check, ChevronDown, Cloud, Code2, Download, Fingerprint, FolderOpen, GitBranch, Laptop, LockKeyhole, MoveRight, Server, ShieldCheck, Smartphone, Terminal } from "lucide-react";
...
const featureIcons = [Server, FolderOpen, Activity, Box, GitBranch, Terminal];
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...meta,
    keywords: locale === "zh-CN" ? ["SSH 客户端", "SFTP 客户端", "服务器管理", "Linux 运维", "AI Terminal"] : ["SSH client", "SFTP client", "server management", "Linux operations", "AI terminal"],
...
export default async function LandingPage({ params }: PageProps) {
  ...
  return <div className="landing" id="top">
    ...
    <main id="main-content">
      ...
      <section className="download-section" id="download"><div className="landing-container"><p className="landing-eyebrow">{t.downloads.eyebrow}</p><h2>{t.downloads.title}</h2><p className="download-description">{t.downloads.description}</p><DownloadPicker copy={t.downloads} /><div className="getting-started" aria-label={t.downloads.start}>{t.downloads.instructions.map((instruction, index) => <div key={instruction}><span>0{index + 1}</span><p>{instruction}</p></div>)}</div><div className="mobile-note"><Smartphone size={22} /><div><h3>{t.downloads.mobile}</h3><p>{t.downloads.mobileDescription}</p></div></div></div></section>
    </main>
