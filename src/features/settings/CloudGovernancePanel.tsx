...
export function CloudGovernancePanel({ organizationId, userId }: { organizationId: string; userId: string }) {
  ...
  return <section className="mt-4 border-t pt-3">
    ...
    {canManage && <div className="mt-3 grid gap-2"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder={t("cloud.policyName")} aria-label={t("cloud.policyName")} /><div className="flex gap-2"><select className="min-w-0 rounded border bg-transparent px-2 text-xs" value={effect} onChange={(event) => setEffect(event.target.value as AccessPolicyEffect)} aria-label={t("cloud.policyEffect")}><option value="deny">{t("cloud.effect.deny")}</option><option value="allow">{t("cloud.effect.allow")}</option></select><select className="min-w-0 flex-1 rounded border bg-transparent px-2 text-xs" value={action} onChange={(event) => setAction(event.target.value as AccessPolicyAction)} aria-label={t("cloud.policyAction")}>{actions.map((value) => <option key={value} value={value}>{t(`cloud.action.${value}`)}</option>)}</select><Button size="icon" disabled={busy || !name.trim()} aria-label={t("cloud.createPolicy")} onClick={() => void addPolicy()}><Plus size={13} /></Button></div></div>}
