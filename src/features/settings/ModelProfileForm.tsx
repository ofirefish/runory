...
import { advancedPresets, chatgptModels, isAdvancedKind, providerLabelKeys } from "./model-provider-presets";
...
export function ModelProfileForm({ profile, busy, onSave, onCancel }: {
  ...
  const models = kind === "chat-gpt" ? chatgptModels : isAdvancedKind(kind) ? advancedPresets[kind].models : [];
  ...
  return <form className="model-profile-form" onSubmit={(event) => {
    ...
    if (busy) return;
    void onSave({ kind, name, baseUrl, model, maxContextTokens, apiKey: apiKey.trim() || null });
  }}>
    <fieldset disabled={busy}>
      ...
      {!oauth && <div className="provider-field"><Label htmlFor={`${formId}-url`}>{t("settings.modelBaseUrl")}</Label><Input id={`${formId}-url`} required type="url" readOnly={kind === "open-ai" || kind === "anthropic" || kind === "google"} value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} placeholder={t("settings.models.endpointPlaceholder")} /></div>}
      {!oauth && <div className="provider-field"><Label htmlFor={`${formId}-key`}>{t("settings.modelApiKey")}</Label>
    ...
    <div className="model-form-footer"><Button variant="ghost" type="button" disabled={busy} onClick={onCancel}>{t("common.cancel")}</Button><Button type="submit" disabled={busy || !model.trim() || (!reuseKey && !apiKey.trim())}>{busy ? <LoaderCircle size={14} className="animate-spin" /> : <Save size={14} />}{t(profile ? "common.save" : "settings.models.save")}</Button></div>
  </form>;
}
