...
export function CloudTeamPanel({ organizationId, userId, onMembershipChanged }: {
  ...
  return <section className="mt-4 border-t pt-3">
    ...
    {canManage && <><div className="mt-3 grid gap-2"><p className="text-xs font-medium">{t("cloud.members")}</p>{members.map((member) => { const editable = member.role !== "owner" && (role === "owner" || member.role === "operator" || member.role === "viewer"); return <div key={member.user_id} className="flex items-center justify-between gap-2 rounded border p-2 text-xs"><span className="min-w-0 flex-1 truncate">{member.email}</span>{editable ? <><SelectControl className="h-8 w-28 shrink-0 px-2 text-xs" value={member.role as InviteRole} disabled={busy} label={t("cloud.memberRole")} onValueChange={(value) => void updateMemberRole(member.user_id, value)} options={(role === "owner" ? (["admin", "operator", "viewer"] as const) : (["operator", "viewer"] as const)).map((value) => ({ value, label: t(`cloud.role.${value}`) }))} /><Button size="icon" variant="ghost" disabled={busy} aria-label={t("cloud.removeMember")} onClick={() => setMemberToRemove(member)}><Trash2 size={13} /></Button></> : <span className="text-[hsl(var(--muted))]">{t(`cloud.role.${member.role}`)}</span>}</div>; })}</div>
