...
export function PrimaryNavigationRail({ active, onSelect }: { active: "servers" | "sessions"; onSelect: (item: "servers" | "sessions" | "settings") => void }) {
  ...
  return <nav className="primary-rail" aria-label={t("shell.primaryNavigation")}>
    ...
    <div className="rail-footer"><button type="button" onClick={() => onSelect("settings")} aria-haspopup="dialog" title={t("sidebar.settings")}><Settings size={20} /><span>{t("sidebar.settings")}</span></button></div>
  </nav>;
}
