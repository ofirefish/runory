...
export function Workspace({ onOpenNavigation, connectProfileRequest }: { onOpenNavigation?: () => void; connectProfileRequest?: { profileId: string; requestId: number } | null }) {
  ...
  return <main className="workspace-shell flex min-w-0 flex-1 flex-col bg-[hsl(var(--background))]">
    ...
    {dialog && dialogProfile && <ConnectionDialog key={dialogProfile.id} profile={dialogProfile} mode={dialog.mode} onClose={() => setDialog(null)} onConnect={(values) => { const tabId = dialog.tabId ?? crypto.randomUUID(); if (!dialog.tabId) setDialog({ ...dialog, tabId }); return openSession(values, tabId, dialog.mode === "reconnect", dialog.title); }} onTest={test} />}
